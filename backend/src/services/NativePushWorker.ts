 ...
 * unchanged here. The one real difference: a `NativePushGoneError`
 * (APNs/FCM reporting the token itself as permanently invalid) deactivates
 * the device token immediately instead of retrying it up to MAX_ATTEMPTS,
 * the same reasoning `PushWorker` applies to a 404/410.
 *
...
export async function processNativePushOutboxOnce(pool: Pool): Promise<number> {
  return runPollingBatch<NativePushOutboxRow>(pool, {
    label: 'Native push outbox',
    attemptsOf: (row) => row.attempts,
    claim: (conn: PoolConnection) =>
      conn
        .query<NativePushOutboxRow[]>(
        ...
        .then(([rows]) => rows),
    deliver: (row) => sendNativePush({ platform: row.platform, token: row.token }, JSON.parse(row.payload)),
    outcome: {
      onSent: (conn, row) =>
        conn
          .execute(
      ...
      onFailure: async (conn, row, attempts, message, err) => {
        ...
        if (gone) {
          await conn.execute(`UPDATE device_push_tokens SET is_active = FALSE WHERE id = ?`, [row.device_token_id]);
        }
        logger.warn(
...
const worker = createPollingWorker('Native push outbox', processNativePushOutboxOnce);
...
export function startNativePushWorker(pool: Pool, pollMs: number = DEFAULT_POLL_MS): void {
  worker.start(pool, pollMs, isNativePushConfigured);
}
