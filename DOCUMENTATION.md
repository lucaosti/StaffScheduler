...
- **`departments.org_unit_id`** — optional FK added in PR #103 to enable org-tree scoping of schedules and shifts, and reused since #602 as the bridge that lets `employee_loans` (scoped to `org_units`) extend a department's scheduling candidate pool.
...
| `approval_matrix` preserved alongside `approval_workflows` | #603 migrated the one remaining request type that used it as its ENTIRE approval mechanism (`PolicyExceptionService`, onto `approval_workflows`/`pending_approvals`, matching `TimeOffService`/`EmployeeLoanService`/`ShiftSwapService`/`ChangeRequestService`). `EmployeeLoanService` and `PolicyExceptionService` still consult `ApprovalMatrixService` narrowly, at creation time only, to decide auto-approval — `approval_steps.auto_approve_for_owner` may express the same thing, which would make the matrix fully redundant, but that call is unmade; the admin CRUD on `policies.ts` and the standalone table stay until it is. |
