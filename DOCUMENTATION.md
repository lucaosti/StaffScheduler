...
One of the six was `limit` on `GET /api/dashboard/activities`, whose handler
took `_req` and hardcoded `LIMIT 10`; it was made a real validated parameter
bounded at 50, and the endpoint itself was later removed altogether (#719) once
it turned out nothing called it and that it duplicated `/api/audit-logs`, which
is what the frontend actually reads that feed from. A reusable parameter no
operation references now fails generation, and `in: 'path'` refs (the
structural `id`) stay hand-written.
...
- **Objective — four lexicographic levels, not a weighted sum.** MEDIUM > DISRUPTION > SOFT > COST. MEDIUM is coverage shortfall; SOFT is employee preferences, workload fairness, a surplus-staffing charge, rest blocks and weekend/night equity; COST is total labour cost. Priority expressed as a ratio between weights is not a guarantee: at the previous coverage-100 / preferences-55, two satisfied preferences outweighed one covered seat, and whether coverage dominated depended on how many preference terms a dataset happened to produce. CP-SAT optimises one scalar, so the ordering is emulated by scaling each level above a bound **summed from the coefficients that actually enter the model below it** — not derived from a weight, which is what made the first attempt wrong (preferences are ±10, not ±1, so soft outranked medium and the solver left shifts empty to satisfy an "avoid"). `optimizer.parity.test.ts` asserts the property directly.
- **Labour cost (soft, CP-SAT only) — a tie-breaker, never a reason to short coverage or unbalance the team.** `users.hourly_rate` exists and feeds the dashboard's monthly-cost figure, but the optimizer never saw it, so two schedules with identical coverage and fairness but very different total cost were indistinguishable to the solver. It doesn't fit the `constraintValidator.ts` checker shape: every other item there measures a *finished* solution, while cost is a preference the search should be steered by *while* solving — for CP-SAT that means a term in the objective function; the greedy engine has no objective to add a term to, so it is not steered by cost at all, the same asymmetry that already exists for workload fairness. It sits at the LOWEST lexicographic level, strictly below SOFT: always assigning the cheapest eligible person would defeat the equity goals SOFT already protects, so cost may only choose BETWEEN solutions already tied on coverage, commitments and fairness. Getting the scaling right needed care beyond the usual "bound the level above": every level ABOVE cost must exceed **both** the rescaled level below it **and** cost's own bound, since cost is summed into the objective independently rather than folded into another level's terms — missing the `+ cost_bound` term reintroduced the exact failure this mechanism exists to prevent, a real fixture leaving a shift unstaffed because covering it cost more than the solver's safety margin allowed. `hourly_rate` never reaches a log line, the solver's stdout, or any API response — it exists only inside the objective function.
...
- **Shift-category rotation across periods (soft), on the same weekend/night categories equity already tracks.** Equity above minimises the *total* spread of category days across employees over the history horizon — it can be perfectly balanced in total while one person still works nights three straight periods running and another has the inverse pattern, since a spread has no notion of adjacency between periods. This charges *consecutive-period concentration* on the same person, independent of the running total: `AutoScheduleService` walks the same predecessor chain `resolvePredecessorId` uses, one period at a time up to a bounded lookback (6 periods), and counts how many of the most recent consecutive **published** periods had the employee working at least one day of the category — stored as `Employee.consecutive_category_periods`, a count of *periods*, not days, which is why it is a field of its own rather than a reuse of `carried_load`. `shiftRotationViolations` in `constraintValidator.ts` flags an employee assigned to a category this period whose streak already meets or exceeds `constraints.max_consecutive_category_periods` (default 2). Soft for the same reason as every other item in this family: made hard, an organization with too few night- or weekend-qualified staff to always rotate becomes unsolvable. Only the CP-SAT engine is steered by it (`_rotation_terms` in `schedule_optimizer.py`, at the same SOFT level as the equity terms it is layered on top of) — the greedy engine has no global view to balance with, the same asymmetry that already applies to weekend/night equity itself.
...
**Module boundary (canonical reference — five modules share adjacent "is this allowed" responsibility; this is where each one's job starts and stops):**

| Module | Scope of ONE call | Called from | NOT its job |
|---|---|---|---|
| `PolicyService` | CRUD + scoped lookup on the `policies` table itself. No enforcement logic — it stores and retrieves rows, and `listApplicable`/`getGlobalValues` answer "which rows apply here", never "is this legal". | `routes/policies.ts`, `PolicyValidator`, `ComplianceEngine`, `CompliancePresetService` | Deciding whether a policy is breached — that is every caller's own job once it has the rows. |
| `PolicyValidator` | One candidate `(user, shift)` pair against every applicable **non-hours** policy (`manual_assignment_locked`, `skill_required`). Reports blocking violations plus whether an approved `PolicyExceptionRequest` already covers each one. | `routes/policies.ts` (`POST /validate/assignment`), `AssignmentService.createAssignment` | Rest-hours/weekly-hours/consecutive-days policies — explicitly routed to `ComplianceEngine` instead (see its own resolution chain below); whole-schedule legality — that's `constraintValidator`'s scope, not a single candidate's. |
| `ComplianceEngine` | One candidate shift against one user's **existing** assignments, for the three working-time rules (`min_rest_hours`, `max_hours_week`, `max_consecutive_days`). Resolves the applicable thresholds through a fixed precedence: employment contract → `user_preferences` → GLOBAL `policies` row → legacy `system_settings` → hardcoded default. Pure rule functions (`checkCompliance`) are DB-free and unit-tested standalone; `evaluateAssignmentCompliance` is the thin DB-aware orchestrator around them. | `AssignmentService.createAssignment`, `ShiftSwapService` (both legs of a swap) | Anything not in the three rules above; whole-schedule legality (see `constraintValidator` below) — this only ever sees one user's own shifts, never staffing caps, skills, or another employee's schedule. |
| `CompliancePresetService` | Applies a **named bundle** of `min_rest_hours`/`max_hours_week`/`max_consecutive_days` policy rows in one call (e.g. `eu_working_time_directive`), upserting by key. Pure convenience over `PolicyService.create` — it contains no rule logic of its own. | `routes/policies.ts` (`POST /presets/:code/apply`) | Enforcement — applying a preset only creates ordinary `policies` rows; `ComplianceEngine` is what makes them count. |
| `constraintValidator` (facade over `optimization/constraints/*`) | A **whole solved schedule** — every employee, every shift, symmetric all-pairs checks (staff caps, double-booking, skills, unavailability, and its own copies of min-rest/max-consecutive-days/max-weekly-hours scoped to the OPTIMIZATION PROBLEM, not the `policies` table). The one authority both scheduling engines (CP-SAT and greedy) are measured against; drift between them is a red test (`optimizer.parity.test.ts`). | `ScheduleOptimizerORTools`, the greedy engine, `optimizer.parity.test.ts` | Reading the `policies` table at all — a solved schedule's legality is judged against the `OptimizationProblem`'s own per-employee contract fields (`max_hours_per_week`, `max_consecutive_days`, …), resolved into that shape once at problem-build time, not against `policies` rows directly. |

**Why `ComplianceEngine` and `constraintValidator` both have a min-rest/max-consecutive-days/max-weekly-hours check, and this is not the duplication it looks like.** They judge different things at different scopes: `ComplianceEngine` is an asymmetric, backward-looking check — one NEW candidate shift against a user's shifts that already exist, called synchronously outside any solve (a manual assignment, a shift swap); `constraintValidator` is a symmetric, whole-schedule check — every pairing among every shift ANY employee holds in one proposed solution, called only from inside the two scheduling engines and their parity suite. Forcing one shape onto the other would not simplify either: `ComplianceEngine` has no "whole schedule" to iterate, and `constraintValidator` has no "candidate vs. existing" asymmetry to preserve. What WAS genuine, accidental duplication — the overnight-aware shift-bounds/duration arithmetic each had reimplemented privately, the exact defect class `optimization/shiftTime.ts`'s own header warns about — is fixed: `ComplianceEngine` now imports `shiftBoundsMs`/`shiftHours` from that module instead of keeping a fourth private copy of overnight-shift math.

`policyKey` is a free-text field on the `policies` table — the catalog is not a fixed enum — but `PolicyValidator.evaluate()` only assigns real blocking behavior to specific keys:
...
| `min_rest_hours`, `max_hours_week`, `max_consecutive_days` | **Enforced.** A GLOBAL policy row for one of these keys now participates directly in `ComplianceEngine.evaluateAssignmentCompliance`'s resolution chain — see the rule-set section below. |
| `skill_required` | **Enforced.** `policy_value: { skillIds: number[] }`. `PolicyValidator.evaluate()` checks the candidate user's `user_skills` against the list and blocks the assignment (surfacing a `PolicyExceptionRequest` prompt like any other blocking policy) when any are missing. |
| `staffing_min` | **Rejected at creation**, not silently accepted. It has no per-assignment meaning — adding one person to a shift can only move it toward a minimum, never away — and the actual minimum-staffing check already exists per-shift as `shifts.min_staff` / `shift_templates.min_staff`, enforced by both scheduling engines' `coverageShortfalls`. `PolicyService.create`/`update` reject this key outright (`ConflictError`) rather than storing a policy that would do nothing; the error message points the caller at `minStaff` on the shift/template instead. |

**Configurable compliance rule sets, seeded from a jurisdiction preset.** `ComplianceEngine.evaluateAssignmentCompliance`'s policy resolution used to stop at the user's employment contract, `user_preferences`, and the legacy `system_settings` keys — never the `policies` table, even though `PolicyValidator` had documented since its own introduction that "the heavy lifting... is performed by the existing ComplianceEngine." An organization's own rule set genuinely participates now: a GLOBAL, active `policies` row for `min_rest_hours` / `max_hours_week` / `max_consecutive_days` slots into the chain between `user_preferences` and `system_settings` — an org-wide regulatory default that a personal contract or preference still overrides, but which now beats the old system-wide fallback. **Not a hardcoded jurisdiction implementation**: the rule is an ordinary row created through the existing `POST/PUT/DELETE /api/policies` CRUD, editable or removable individually like any other policy. `CompliancePresetService` (`GET /api/policies/presets`, `POST /api/policies/presets/:code/apply`, both `policy.manage`-gated) is a convenience for loading a **named bundle** of these rows in one call rather than typing three by hand — today just `eu_working_time_directive` (11h daily rest, 48h average week, 6 consecutive working days), upserting by key so re-applying never duplicates a row and a rule under a different key the organization already customized is left untouched. Adding FLSA or Fair Workweek later means adding another entry to `COMPLIANCE_PRESETS`, not new enforcement code.
...
**Attendance anomalies, by contrast, are computed at query time rather than persisted** — `AttendanceService.detectAnomalies` (behind `GET /api/reports/attendance-anomalies`, same gate) — because nothing about a punch is at risk of being lost the way an unrecorded compliance violation was: `attendance_records` and `shifts` already hold everything an anomaly is derived from, so a second table would only cache a query rather than record a fact that would otherwise vanish. All four patterns are flagged in one pass — late clock-in, a clock-out left open well past the shift's end, an early clock-out, and a punch with no matching assignment at all — each result referencing the underlying `attendance_records` row (not just a computed dashboard figure), so a later reconciliation-or-payroll-authorization step can act on the exact punch. The two threshold-based patterns (late/early) read their grace period from `system_settings` (category `attendance`), organization-configurable rather than a hardcoded cutoff, consistent with how this system treats other thresholds elsewhere. The arithmetic runs as `TIMESTAMPDIFF` in SQL rather than parsing a MySQL `DATETIME` string back into a JavaScript `Date` and guessing its timezone — the same reasoning `SHIFT_HOURS_SQL`/`SHIFT_ABS_END_SQL` exist for elsewhere in this codebase.
...
**Same transactional-outbox shape as email/push**: `WebhookService.dispatch(organizationName, eventType, payload, conn?)` enqueues one `webhook_deliveries` row per active matching subscription — the optional `conn` lets a call site enqueue inside its own transaction, the same seam `NotificationService.notifyWithin` offers. Call sites are best-effort and fire-and-forget (`.catch(...)`, not awaited): `ScheduleService.publishSchedule`, `AssignmentService.confirmAssignment`, and `ApprovalDecisionService.decidePendingApproval` (the last one takes `organizationName` as an explicit optional parameter supplied by its callers — see below — rather than looking it up itself, since it is a shared method used by four unrelated entity services and an unconditional lookup query there would run on every call regardless of whether webhooks are in play).

**`organizationName` threading**: `ApprovalDecisionService.decidePendingApproval` is invoked by `TimeOffService`, `ShiftSwapService`, `EmployeeLoanService`, `PolicyExceptionService`, and `ChangeRequestService`, all of which now accept `organizationName` as a trailing optional parameter (default `null`, meaning "skip dispatch") and forward it unchanged. Routes supply it from `req.user.organizationName` — already resolved by `authenticate` — so no extra query is needed at the HTTP boundary either.
...
## 7g. Mobile app (Capacitor)

`mobile/` is an npm workspace member wrapping the existing frontend in
[Capacitor](https://capacitorjs.com/) to produce native iOS and Android app shells,
reusing the frontend's component/hook/service layer entirely rather than maintaining a
second UI codebase (the technology decision behind this: Capacitor over Expo/React
Native, see the architectural decision that motivated this workspace).

**Layout**: `mobile/capacitor.config.ts` declares `appId: 'com.staffscheduler.app'`,
`appName: 'Staff Scheduler'`, and `webDir: 'www'`. `webDir` is local to `mobile/`, not a
path into `../frontend/build`, because Capacitor resolves `webDir` relative to the
Capacitor project itself and expects the web assets to already live inside the native
project tree it generates there — pointing `webDir` at a sibling directory works for
`cap sync` but breaks `cap open ios`/`cap open android`. The standard pattern for a
monorepo with the web app in a sibling package is therefore a build step that builds the
frontend, copies its output into a local `www/`, then syncs — not fighting the tool into
reading `webDir` from outside its own project.

**Scripts** (`mobile/package.json`):

```bash
npm run build --workspace=mobile   # frontend build -> copy into mobile/www -> cap sync
npm run sync --workspace=mobile    # cap sync only (both platforms)
npm run open:ios --workspace=mobile      # open the Xcode project
npm run open:android --workspace=mobile  # open the Android Studio project
```

`build` runs `npm --prefix ../frontend run build` (not `npm run build --workspace=frontend`,
which only resolves from a workspace-aware root context, not from inside a package
directory), then `mobile/scripts/copy-web-assets.mjs` (copies `frontend/build` over
`mobile/www`), then `npx cap sync`.

**Current state (honest, as of this workspace's introduction)**:

- **iOS**: fully scaffolded — `mobile/ios/` is a real Xcode project (`App.xcodeproj`),
  generated by `npx cap add ios` and kept current by `cap sync`. Capacitor 8's default iOS
  template uses Swift Package Manager (`ios/App/CapApp-SPM`) rather than CocoaPods, so no
  CocoaPods installation was needed to scaffold or sync it in this environment.
- **Android**: scaffolded — `mobile/android/` is a real Gradle project
  (`build.gradle`, `settings.gradle`, the `app/` module, the Gradle wrapper), generated by
  `npx cap add android`. Its automatic Gradle sync step failed in the environment this
  workspace was scaffolded in (no Java runtime installed), so the project has not yet been
  Gradle-synced or built; a machine with a JDK (and, to build/run it, the Android SDK) needs
  to open it in Android Studio or run `./gradlew` once to complete that step. The generated
  files themselves are unaffected by that — `cap add`/`cap sync` do not require Gradle to
  succeed to write the project.
...
- `responsibility_rule` — resolves approvers dynamically via the responsibility matrix; requires `approverPermissionCode` on the step; `ApproverResolutionService.resolveAllApproversForStep(step, ctx)` returns the full set for fan-out notifications

Every `pending_approvals` row belongs to exactly one entity — `change_request_id`, `time_off_request_id`, `employee_loan_id`, `shift_swap_request_id`, or `policy_exception_id` (a `CHECK` constraint enforces exactly one is set). Time-off, employee-loan, shift-swap, and policy-exception approve/reject now route through the same `ApprovalDecisionService.decidePendingApproval` as change requests, instead of each having its own bespoke authorization check. `policy_exception_id` was the last holdout: `PolicyExceptionService` previously routed exclusively through the legacy `approval_matrix`/`ApprovalMatrixService` — the entire approval mechanism for that one request type, not the narrow creation-time auto-approve fast path `EmployeeLoanService` and `PolicyExceptionService` both still make of it — so it alone lacked the ordered multi-step routing, structure delegation, and responsibility rules the other four already had.
...
**Authorization**:
- Deciding a `unit_structure` item: the current assignee, or (once opened) any member of `assigned_to_org_unit_id` — same `ApprovalDecisionService.decidePendingApproval` check used for person-assigned decisions.
...
| An SSE stream listens for its own `error`, not just `close`/`end` (#556) | `routes/events.ts`'s heartbeat wrote to `res` inside a `try`/`catch` and cleaned up on the request's `close`/`end` events — which covers a graceful disconnect but not every way a long-lived stream ends. Writing to an already-destroyed response does not always throw synchronously; Node can instead emit `'error'` on the stream asynchronously, and an emitted `'error'` with no listener is not a normal rejection — EventEmitter re-throws it as an uncaught exception, on whatever call stack happens to be running when the event fires. On an unref'd 30-second heartbeat interval that stack has nothing to do with the request that armed it, so the crash surfaced as an unrelated, unrepeatable failure in whichever suite happened to be executing at that moment — the "#556" flake: under `jest --coverage` with parallel workers, a small fraction of runs failed one arbitrary route suite with a symptom that varied by run, and always passed in isolation because the interval never got 30 seconds to fire in one file's tests alone. Both `req` and `res` now also listen for `'error'` and route it through the same idempotent `cleanup()`, closing the gap `try`/`catch` cannot: a synchronous throw and an asynchronous `emit('error', ...)` are different failure shapes and need different handlers, not one doing both jobs. The regression pins used a real `EventEmitter`-backed fake response (`res.emit('error', ...)` synchronously invoking the listener) rather than a real socket, since forcing Node's actual write-after-destroy timing is exactly the nondeterminism under test; the real-HTTP-server integration test was also tightened to await `eventBus.unsubscribe` actually firing rather than the client socket's own `close`, since that race — server-side cleanup not yet run when the test tore down its server — is what let the interval outlive its test in the first place. |
...
| `usingConnection` owns the connection release; `withTransaction` owns begin/commit/rollback — but the 45 sites with post-commit work were not forced onto the latter | 23 services each hand-wrote `getConnection` → `try` → `finally { release() }`, 52 times. All 52 were correct, so this was duplication rather than a defect — but it was 52 independent chances to omit the `finally`, and omitting it leaks a pooled connection **on the error path only**: it survives every happy-path test and surfaces later, under load, as pool exhaustion with nothing in the stack naming the culprit. `usingConnection` makes the release structural. The obvious stronger helper — one that also runs the transaction — was measured against the call sites before being adopted and applies cleanly to only 7 of them: in 45, work continues *after* `commit()` inside the same `try` (a post-commit read-back, an audit write, a log line), so handing the boundary to the helper would silently move that work inside the transaction. Splitting each at its commit boundary is 45 individual judgements about what is atomic, in code whose tests mock the connection and mostly do not assert ordering relative to `commit()`. So `withTransaction` exists, `Database.transaction()` delegates to it, and new units of work that genuinely end at the commit use it — while the existing 45 keep begin/commit/rollback visible at the call site, where a reader can see exactly what is atomic (#721). Two sites keep the fully explicit form because a callback would change their control flow, not just their shape: `UserDirectoryService.importFromVcf` uses `continue` inside a loop over vCards, and `ShiftSwapService.respondAsTarget` returns early while further work follows. `withTransaction` additionally guards its own rollback: a rollback failing means the *server* is the problem, exactly when the original error is most needed, so the original always propagates and the rollback failure is logged instead of replacing it. |
| Three dashboard endpoints were deleted rather than kept as a documented API | `GET /api/dashboard/activities`, `/upcoming-shifts` and `/departments` had no caller anywhere in the repository — not the SPA, not `mobile/` (which wraps the built frontend and issues no requests of its own), not a script or seed. The only references were their own route definitions, their OpenAPI entries, the auto-generated frontend types nothing imported, and the tests exercising them. `/activities` additionally duplicated `GET /api/audit-logs`, which is what `dashboardService.getRecentActivity` actually calls for that widget. Keeping them would mean maintaining, testing and specifying ~130 lines of route code plus three spec entries for a surface with no consumer, and every future schema change would have had to keep them working. Removed with their spec entries and tests (#719); the surviving `/stats` and `/attention-items` moved to `DashboardService`, which is what made `routes/dashboard.ts` the last router in the codebase to own raw SQL — it held 10 statements where every other route file holds at most one. |
| Employee loans and employment contracts only began affecting generated schedules once the period stopped being converted with `String().slice(0, 10)` | `AutoScheduleService` converted the schedule period to `YYYY-MM-DD` for its two string-typed lookups with `String(schedule.start_date).slice(0, 10)`. `schedules.start_date` is a `DATE` column and the pool sets no `dateStrings`, so mysql2 materialises it as a JavaScript `Date` — and slicing ten characters off `String(new Date(…))` yields `'Fri May 01'`, which MySQL cannot parse as a date. Both lookups therefore matched nothing, silently: `EmployeeLoanService.listLoanedInUserIds` returned no one, so approving a loan never actually made that person schedulable in the destination department (the exact outcome the `departments.org_unit_id` bridge was built to produce), and `EmploymentContractService.resolveLimitsForPeriod` returned an empty map, so every employee fell back to the `user_preferences` defaults that contracts exist to supersede. Neither failure was visible: both degraded to a plausible schedule built on the wrong limits. Fixed by using `DateUtils.toDateString`, which handles both the `Date` and the string shape and whose own doc comment names this failure mode — this service was the last site in the codebase still doing the conversion by hand (#716). The 933-line test suite missed it because every fixture states these columns as strings, the one shape where the hand-rolled conversion is accidentally correct; the regression tests now feed `Date` objects, as the driver really does. |
