...
| `staffing_min`, `skill_required` | **Not enforced by the policy engine.** Accepted by `POST /api/policies` and stored, but `PolicyValidator` treats them as informational only — creating one does not block anything, and no other mechanism enforces them either. |

Implication: an administrator who creates a `staffing_min` or `skill_required` policy through the Policies UI gets no error, but the policy has no effect — a known gap, not yet surfaced in the UI. Extending `PolicyValidator.evaluate()` to cover them is open work.
