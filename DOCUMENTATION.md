...
- **Weekend equity (soft).** The hours fairness balances *how many* hours people work; this balances *which* hours. A schedule can be perfectly even by total load while one person works every Saturday and Sunday and another works only weekdays — to an hours-only measure a Sunday hour and a Tuesday hour are the same hour, and the complaint that gets raised is usually raised by whoever is most available, because that is exactly who a scheduler with no weekend term keeps assigning. Modelled as the spread in weekend **days** (not hours: a four-hour Sunday shift costs the day either way), counting a day once however many shifts it holds, and counting work held on other schedules. Which days count is configurable via `constraints.weekend_days` — Saturday/Sunday is a **default, not a truth**, since several sectors this system targets run rotas where the unsocial days differ.
