...
**Not yet wired end-to-end**: `routes/twoFactor.ts`'s `/setup` and `/enable` still don't accept a `methodType` from the client (always `'totp'`), and `routes/auth.ts`'s `/login` still only checks and verifies the `'totp'` provider — see #591, filed once it became clear both need to land together (letting a user enroll in a non-TOTP method without teaching login to recognize it would either skip the challenge entirely or demand a `totpCode` no provider exists to check, either a security hole or a lockout). #591 was intentionally left until TOTP+email+WebAuthn all existed, so the routes/login generalization gets designed once against three real provider shapes rather than incrementally against one.
